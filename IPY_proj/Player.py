...
class Player:
    def __init__(self, field_lgth):
        ...
        self._last_cell_coords = (-1, -1)  # выбранная, но не подтвержденная клетка               
        self._last_cell = None    
    # Записывает значение, поставленное в эту клетку поля, в матрицу
    def save_the_value(self, value):
        self._field[self._last_cell_coords[0]][self._last_cell_coords[1]] = value
        self.move_confirmed = False
        self._last_cell_coords = None        

    def tmp_cell_chosen(self, cell, coords):
        if self._last_cell is not None:
            self._last_cell.Content = ""
        self._last_cell = cell
        self._last_cell_coords = coords        
